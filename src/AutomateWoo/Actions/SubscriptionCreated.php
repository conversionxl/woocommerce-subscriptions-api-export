...
/**
 * Subscription: Export new subscription to ChartMogul.
 *
 * @since 2021.06.22
 * @package CXL\WC\ChartMogul\AutomateWoo\Actions
 */
class SubscriptionCreated extends Action {
	...
	public function run(): void {
		...
		Logger::log()->debug( 'Export initiated by AutomateWoo: Subscription created', [
		...
		try {
			...
			if ( 'pong!' !== CMComponent::ping() ) {
				...
				$this->workflow->log_action_error( $this, 'Exporting new subscription failed. Please verify action config.' );
				return;
			}
	...
	protected function load_admin_details(): void { // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
		$this->title = 'Export new subscription';
	...
	protected function log_success(): void { // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
		$this->workflow->log_action_note( $this, 'New subscription exported.' );
	}
