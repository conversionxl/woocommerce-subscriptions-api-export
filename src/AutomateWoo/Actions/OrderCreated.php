...
/**
 * Order: Export new order to ChartMogul.
 *
 * @since 2021.06.22
 * @package CXL\WC\ChartMogul\AutomateWoo\Actions
 */
class OrderCreated extends Action {
	...
	public function run(): void {
		...
		Logger::log()->debug( 'Export initiated by AutomateWoo: Order created', [
		...
		try {
			...
			if ( 'pong!' !== CMComponent::ping() ) {
				...
				$this->workflow->log_action_error( $this, 'Exporting new order failed. Please verify action config.' );
				return;
			}
	...
	protected function log_success(): void { // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
		$this->workflow->log_action_note( $this, 'New order exported.' );
	}
